...
export const PopupContextProvider = ({children, storageManager}: PopupContextProviderProps) => {

    const [siteItems, setSiteItems] = useState<ISiteItem[]>([])

    const addSite = (domain: string) => {
        storageManager.addSite(domain)
        .then(storageManager.getSiteList)
        .then(setSiteItems)
    }

    const removeSite = (domain: string) => {
        storageManager.removeSite(domain)
        .then(storageManager.getSiteList)
        .then(setSiteItems)
    }

    const toggleSiteStatus = (domain: string) => {
        storageManager.toggleSiteStatus(domain)
        .then(storageManager.getSiteList)
        .then(setSiteItems)
    }
